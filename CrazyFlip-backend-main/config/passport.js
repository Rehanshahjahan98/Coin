...
passport.use(new LocalStrategy({ usernameField: 'walletAddress' }, (walletAddress, password, done) => {
	User.findOne({ walletAddress: walletAddress.toLowerCase() }, (err, user) => {
		if (err) { return done(err) }
		if (!user) {
			return done(null, false, { msg: `walletAddress ${walletAddress} not found.` })
		}
		if (!user.password) {
